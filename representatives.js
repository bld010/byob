...
const getStates = () => {
  ...
  cleanedRepresentatives.forEach(representative => {
    if (!states.includes(representative.state)) {
      states.push(representative.state)
    }
  })
  return states;
}
...
const populateSenatorsInStates = () => {
  let stateObjects = createStateObjects();

  stateObjects.forEach(stateObject => {
    
    let matchingSenators = senators.filter(senator => senator.state === stateObject.abbreviation);

    stateObject.senators = matchingSenators;
  })
...
const populateRepresentativesInStates = () => {

  let stateObjects = populateSenatorsInStates();

  stateObjects.forEach(stateObject => {
    let matchingReps = cleanedRepresentatives.filter(rep => rep.state === stateObject.abbreviation);

    stateObject.representatives = matchingReps;
  })
